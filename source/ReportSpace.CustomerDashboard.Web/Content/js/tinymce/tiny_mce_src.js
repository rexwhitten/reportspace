...
(function(namespace, expando) {
	...
	function fix(original_event, data) {
		...
		// Add stopPropReportSpacetion
		event.stopPropReportSpacetion = function() {
			event.isPropReportSpacetionStopped = returnTrue;

			// Execute stopPropReportSpacetion on the original event object
			if (original_event) {
				if (original_event.stopPropReportSpacetion) {
					original_event.stopPropReportSpacetion();
				} else {
		...
		// Add stopImmediatePropReportSpacetion
		event.stopImmediatePropReportSpacetion = function() {
			event.isImmediatePropReportSpacetionStopped = returnTrue;
		...
		if (!event.isDefaultPrevented) {
			...
			event.isPropReportSpacetionStopped = returnFalse;
			event.isImmediatePropReportSpacetionStopped = returnFalse;
		}
	...
	function EventUtils(proxy) {
		...
		function executeHandlers(evt, id) {
			...
			if (callbackList) {
				for (i = 0, l = callbackList.length; i < l; i++) {
					...
					// Should we stop propReportSpacetion to immediate listeners
					if (evt.isImmediatePropReportSpacetionStopped()) {
						return;
					}
				}
			}
		...
		self.fire = function(target, name, args) {
			...
			} while (target && !event.isPropReportSpacetionStopped());
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getNode : function() {
			...
			// Range maybe lost after the editor is made visible ReportSpacein
			if (!rng)
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		_refreshContentEditable : function() {
			...
			// Check if the editor was hidden and the re-initalize contentEditable mode by removing and adding the body ReportSpacein
			if (self._isHidden()) {
				body = self.getBody();
			...
			function removeRngStyle(rng) {
				...
				if (format.split) {
					...
					if (startContainer != endContainer) {
						...
						// Unwrap start/end to get real elements ReportSpacein
